...
class File extends Base {
    ...
    }; file () { try { return require (this.path[0]) } catch(error) { return this.errors.push (error) }
    }; cache () {
        if (!this.file()) throw new Error ("Attempted to grab cache from an unloaded file.");
        return require.cache[require.resolve (this.path[0])];
    }; static refresh (file, cache = false) {
        let _cache = [cache]; delete require.cache[require.resolve (this.path[0])];
        if (!cache) return file.file (); require.cache[require.resolve (this.path[0])] = _cache[0]; return file.file ();
    };
}; exports.main = File;
