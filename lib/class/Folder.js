"use strict"; const Path = require ("../func/path.js"); const Base = require ("./Base.js").main; const File = require ("./File.js").main;
const fs = require ("fs"); // Comment this entire line out if you have fs defined as a global variable, if you don't know what that means then don't touch this.
...
class Folder extends Base {
    constructor (path, options) {
        ...
        try {
            ...
            options = Object.assign ({ }, { blacklist: Path.blacklist, whitelist: /^$/, endless: false }, options || { });
            let children = fs.readdirSync (this.path[0]).filter (child => (!child.match (options.blacklist) && child.match (options.whitelist)) || include.includes (child));
            ...
            this.children = this.children.filter (child => child); for (let i in children) {
                ...
                child.type = require ("../func/typeof.js").main (fs.lstatSync (child.path));
                if (child.type[1]) throw new Error ("unknown child type");
                (async function () {
                    this[`/${child.name}`] = new child.type (child.path);
                    if (child.type == Folder && options.endless === true) this.child[`/${child.name}`].loadChildren (options);
                }()); Object.defineProperty (this.children, "size", {
